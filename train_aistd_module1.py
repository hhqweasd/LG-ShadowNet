...
from model_module1 import Generator_F2S,Generator_S2F,Discriminator
...
parser.add_argument('--snapshot_epochs', type=int, default=10, help='number of epochs of training')
parser.add_argument('--iter_loss', type=int, default=100, help='average loss for n iterations')
opt = parser.parse_args()

# USR
#opt.dataroot = '/home/liuzhihao/dataset/shadow_USR'
# SRD
...
opt.log_path = os.path.join('model_istda_lgsn_module1', str(datetime.datetime.now()) + '.txt')
...
for epoch in range(opt.epoch, opt.n_epochs):
    for i, batch in enumerate(dataloader):
        ...
        mask_queue.insert(mask_generator_lab(real_A, fake_B))
    ...
    torch.save(netG_A2B.state_dict(), 'model_istda_lgsn_module1/netG_A2B.pth')
    torch.save(netG_B2A.state_dict(), 'model_istda_lgsn_module1/netG_B2A.pth')
    torch.save(netD_A.state_dict(), 'model_istda_lgsn_module1/netD_A.pth')
    torch.save(netD_B.state_dict(), 'model_istda_lgsn_module1/netD_B.pth')

    if (epoch + 1) % opt.snapshot_epochs == 0:
        torch.save(netG_A2B.state_dict(), ('model_istda_lgsn_module1/netG_A2B_%d.pth' % (epoch + 1)))
        torch.save(netG_B2A.state_dict(), ('model_istda_lgsn_module1/netG_B2A_%d.pth' % (epoch + 1)))
        torch.save(netD_A.state_dict(), ('model_istda_lgsn_module1/netD_A_%d.pth' % (epoch+1)))
        torch.save(netD_B.state_dict(), ('model_istda_lgsn_module1/netD_B_%d.pth' % (epoch+1)))
