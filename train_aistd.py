...
from utils import QueueMask_llab
from model_lgsn import Generator_F2S,Generator_S2F,Discriminator
...
if not os.path.exists('model_istda_lgsn'):
    os.mkdir('model_istda_lgsn')
opt.log_path = os.path.join('model_istda_lgsn', str(datetime.datetime.now()) + '.txt')

print(opt)
    
###### Definition of variables ######
# Networks
netG_A2B = Generator_S2F.from_file('./AISTD/Module1/netG_A2B.pth')  # shadow to shadow_free
netG_B2A = Generator_F2S.from_file('./AISTD/Module1/netG_B2A.pth')  # shadow_free to shadow
...
optimizer_G = torch.optim.Adam(filter(lambda p:p.requires_grad, itertools.chain(netG_A2B.parameters(), netG_B2A.parameters())),lr=opt.lr, betas=(0.5, 0.999))
...
mask_queue =  QueueMask_llab(dataloader.__len__()/4)
open(opt.log_path, 'w').write(str(opt) + '\n\n')

def colour_angle(ff,rr):
    return torch.mul(ff,rr).sum(1)/(torch.mul(torch.pow((torch.pow(ff,2)).sum(1),0.5),torch.pow((torch.pow(rr,2)).sum(1),0.5))+1e-8)
...
for epoch in range(opt.epoch, opt.n_epochs):
    for i, batch in enumerate(dataloader):
        ...
        same_A,_ = netG_B2A(real_A,real_A_L,mask_non_shadow,mask_non_shadow)
        ...
        recovered_A,_ = netG_B2A(fake_B,fake_B_L,cmask,cmask_L) # real shadow, false shadow free
        ...
        if (i+1) % opt.iter_loss == 0:
            log = 'Epoch: %d, [iter %d], [loss_G %.5f], [loss_G_identity %.5f], [loss_G_GAN %.5f],' \
                  '[loss_G_cycle %.5f], [loss_D %.5f], [loss_colour_A %.5f], [loss_colour_B %.5f]' % \
            ...
            outputshadowfreeimage=fake_B.data
            outputshadowfreeimage[:,0]=50.0*(outputshadowfreeimage[:,0]+1.0)
            outputshadowfreeimage[:,1:]=255.0*(outputshadowfreeimage[:,1:]+1.0)/2.0-128.0
            outputshadowfreeimage=outputshadowfreeimage.data.squeeze(0).cpu()
            outputshadowfreeimage=outputshadowfreeimage.transpose(0, 2).transpose(0, 1).contiguous().numpy()
            outputshadowfreeimage=resize(outputshadowfreeimage,(480,640,3))
            outputimagerealsr=color.lab2rgb(outputshadowfreeimage)
            io.imsave('./model_istda_lgsn/outputshadowfreeimage.png',outputimagerealsr)
    ...
    torch.save(netG_A2B.state_dict(), 'model_istda_lgsn/netG_A2B.pth')
    torch.save(netG_B2A.state_dict(), 'model_istda_lgsn/netG_B2A.pth')
    torch.save(netD_A.state_dict(), 'model_istda_lgsn/netD_A.pth')
    torch.save(netD_B.state_dict(), 'model_istda_lgsn/netD_B.pth')

    if (epoch+1)>90:
        torch.save(netG_A2B.state_dict(), ('model_istda_lgsn/netG_A2B_%d.pth' % (epoch + 1)))
        torch.save(netG_B2A.state_dict(), ('model_istda_lgsn/netG_B2A_%d.pth' % (epoch + 1)))
        torch.save(netD_A.state_dict(), ('model_istda_lgsn/netD_A_%d.pth' % (epoch+1)))
        torch.save(netD_B.state_dict(), ('model_istda_lgsn/netD_B_%d.pth' % (epoch+1)))
